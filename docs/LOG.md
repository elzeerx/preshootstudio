...
**Mission 8: تفعيل تبويب المقال**

- إضافة حقول article في جدول Project:
  - `article_status` (TEXT)
  - `article_last_run_at` (TIMESTAMP WITH TIME ZONE)
  - `article_data` (JSONB)

- إنشاء System Prompt للمقال في `systemPrompts.ts`

- إنشاء أنواع البيانات للمقال في `src/lib/types/article.ts`

- إنشاء Supabase Edge Function: `run-article`
  - استخدام نموذج AI لتوليد مقال جاهز للنشر
  - تخزين النتيجة في قاعدة البيانات

- تحديث `ArticleTab.tsx` بالكامل:
  - عرض حالات (idle, loading, error, ready)
  - إمكانية توليد المقال بالذكاء الاصطناعي
  - عرض المقال بتنسيق منظم (عنوان، مقدمة، أقسام، خاتمة)
  - أزرار نسخ (نص عادي + Markdown)
  - عرض معلومات SEO
...
**Mission 9: تفعيل تبويب التصدير**

- إنشاء أنواع البيانات للتصدير في `src/lib/types/export.ts`:
  - `PreShootExportPack`: يحتوي كل بيانات المشروع + Markdown

- إنشاء Supabase Edge Function: `run-export`
  - تجميع كل مخرجات المشروع (البحث، السكريبتات، B-Roll، البرومبتات، المقال)
  - بناء ملف Markdown شامل منظم
  - لا يستدعي AI جديد، يعتمد فقط على البيانات المخزنة

- تحديث `ExportTab.tsx` بالكامل:
  - عرض حالة جاهزية كل قسم مع أيقونات وألوان
  - زر "تجهيز حزمة PreShoot"
  - معاينة Markdown في textarea قابل للتمرير
  - أزرار تحميل (Markdown + JSON)
  - أزرار نسخ وإعادة تجهيز
  - معالجة حالات التحميل والأخطاء

*آخر تحديث: Mission 9 - تفعيل تبويب التصدير*
